...
PeerConnection::PeerConnection(EventLoop *el, PortAllocator *allocator)
    ...
    _transport_controller->signal_candidate_allocate_done.connect(
...
int PeerConnection::set_remote_sdp(const std::string &sdp) {
    ...
    for (auto field : fields) {
        if(field.find("m=group:BUNDLE") != std::string::npos){ // 是否存在BUNDLE
            std::vector<std::string> items;
            if(is_rn){
        ...
        } else if (field.find("m=") != std::string::npos) { // 是否存在m行
