...
// 事件回调
void async_udp_socket_io_cb(EventLoop * /*el*/, IOWatcher * /*w*/, int /*fd*/,
...
AsyncUdpSocket::AsyncUdpSocket(EventLoop *el, int socket)
    : _el(el), _socket(socket), _buf(new char[MAX_BUF_SIZE]), _size(MAX_BUF_SIZE){
...
int AsyncUdpSocket::send_to(const char* buf, size_t len, const rtc::SocketAddress& addr){
    return _add_udp_packet(buf, len, addr);
}

int AsyncUdpSocket::_add_udp_packet(const char* data, size_t len, const rtc::SocketAddress& addr){
  UdpPacketData* packet = new UdpPacketData(data, len, addr);
